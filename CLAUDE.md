...
All four previously-open Phase 1 items are now addressed (BSE history, liquidity filter, security master, corporate actions, fundamentals-metadata) — remaining known gaps are narrow and explicitly documented above (BSE suspension/delisting detection, XBRL line-item parsing, yfinance fallback), not silently missing.
