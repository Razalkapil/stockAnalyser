...
- **2026-09-19 — The TradingView embed is dropped; the stock view links out.** The plan's open question ("does the free widget render NSE/BSE symbols?") was answered empirically in headless Chrome: it does not (NSE: "only available on TradingView"; BSE: no data; scrip codes: "doesn't exist"). The stock view keeps only the Lightweight-Charts view of our adjusted bars plus an "Open on TradingView" link, and the CSP allows no third-party origin.
- **2026-09-19 — Fundamentals come from two NSE filing systems.** The legacy `corporates-financial-results` feed ends at the Dec 2024 quarter; the Integrated Filing system covers Mar 2025 onward and is now swept too (same parser). Not in the original plan; found when a real sweep showed no filing newer than 2024-12-31.
