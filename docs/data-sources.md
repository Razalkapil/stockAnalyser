...
- **What is still NOT adjusted (and flagged `degraded` on every rebuild):** **rights issues** (62) and **demergers** (20) — both move prices, and a rights factor needs the issue price (face value + the `@ Premium` amount) and the cum-price. Cash events (dividends, distributions, bond interest) and events with no price effect (buybacks, AGMs) are deliberately *not* counted as degradation; flagging them would raise a permanent false alarm.
