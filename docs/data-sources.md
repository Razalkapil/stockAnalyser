...
### Symbol changes — `symbolchange.csv` (verified live 2026-09-19)

```
https://nsearchives.nseindia.com/content/equities/symbolchange.csv
```

- **No header row.** Four columns: company name, old symbol, new symbol, date (`DD-MON-YYYY`, e.g. `05-AUG-2022`). 1,060 rows back to 2003, unsorted. Company names can be blank or start with a space. `text/csv`, browser UA only, no cookie. Because there is no header, the fetch only demands a comma in the first line; the real shape check is per row (exactly 4 fields, both symbols non-blank, a parseable date) and any deviation raises.
- **Why it is needed:** EQUITY_L only lists today's symbols, so `symbol_history` built from master snapshots starts at the first snapshot (2026-09-18). A rename before that was invisible: UNOMINDA's 2022 bonus never reached the MINDAIND bars (a phantom 49% crash); TIPSMUSIC's 2023 split never reached TIPSINDLTD.
- `stk ingest symbol-changes` (weekly, after `master`): links each old symbol to the security holding the new one — newest first, so chains resolve (TELCO → TATAMOTORS → TMPV). First run: **491 old symbols linked, 568 for securities no longer listed** (normal). An old symbol since reused by *another* security is never linked (it would apply one company's actions to another's bars) — only **DTIL** so far. `valid_from` is the change into the old symbol when the file has one, else `1900-01-01` (since listing, unknown).
- **It also classifies old ETF symbols.** `instrument_class` comes from UDiFF, which starts 2024-07, so an ETF renamed before then (ICICI500 → BSE500IETF, NETFAUTO → AUTOBEES) was treated as a *stock* in backtests. Old symbols now inherit the class of the symbol they became: **625 classified**, unclassified EQ symbols since 2021 fell 407 → 171. **Remaining gap:** ~14 ETFs that matured or delisted without a rename (e.g. `ICICIBANKN`, `SDL24BEES`, `EBBETF0425`) still have no class and still enter the stock universe — no free source found that lists them; a name heuristic was deliberately not used.
...
Findings from the probe, and from running it for real:
...
- **Non-trading dates 404 honestly** — unlike BSE, there is no silent-200 SPA shell here. Weekends, a real holiday (2026-01-26) and a nonsense date (`31129999`) all returned a clean 404 with `text/html`. The requested-date assertion (`assert_index_bars_match_requested_date`) is applied because ADR 0003 showed that assumption failing on a sibling archive — and it has since fired here too, see the next-but-one finding.
- **`Nifty50 Dividend Points` is a cumulative counter, not a price, and RESETS TO ZERO every financial year** *(found 2026-09-21 from 65 failed ingest days)*. It carries only a close (`-` for open/high/low) and reads e.g. 289.38 on 2025-03-27, then `0.0` on the first sessions of April, then 4.22 by 2025-04-28. A `close <= 0` assertion treated that zero as corrupt and aborted the whole day, so the benchmark lost 65 trading days across 2021–2026, every one in the first weeks of April. `assert_index_bars_sane` now allows a zero close only for a bar with no open/high/low (a derived series, told apart by what it carries rather than by name); a negative close, or a zero on a price-level index, is still fatal. Fixture: `tests/fixtures/nse/ind_close_all_01042025_fy_boundary.csv` (a real, trimmed file). The 64 recoverable days were re-parsed from stored bytes with `stk ingest indices --reparse` and no network.
- **Three days in April 2023 print their date MONTH-first** *(2023-04-06, 04-10, 04-11)*. The file served for 6 April 2023 says `04-06-2023` on every row, i.e. it parses as 4 June; NSE returned to day-first from 04-12. `assert_index_bars_match_requested_date` correctly refuses all three. They are left as **open gaps** rather than auto-corrected: swapping day and month on a heuristic is a judgement about NSE's intent that this project does not make silently. `stk doctor` reports them as `benchmark_gap`. Each sits inside a half-yearly walk-forward window, and alpha uses window endpoints, so no gate verdict depends on them.
- **There was no range fill for indices** until `stk backfill indices --from --to` (2026-09-21). A hand-run loop had stopped early and left 2023-04-10 → 2023-05-22 unfetched with nothing reporting it; the backtest forward-fills its benchmark, so the hole read as a flat market for a month. `stk doctor`'s `benchmark_gap` check now names any trading day that has prices but no benchmark close.
...
- **Real-data verification, 2026-09-19** (3,180 subjects from `--since 2025-06-01`). The parser as first written handled only hand-written examples and left **1,029 of 3,180 unparsed** — including **62 real stock splits** (a 5:1 split reads as an 80% crash if unadjusted). Root causes, all now fixed (parser v2): the singular rupee `Re` (`Dividend - Re 1 Per Share`, ~600 rows); NSE truncating subjects to `Per Sh`; the wording `Face Value Split (Sub-Division) - From Rs 10/- Per Share To Rs 2/- Per Share`; `Consolidation Of *Equity* Shares From Re 1 Per Share To Rs 10 Per Share`; InvIT/REIT `Distribution - Rs 3.75 Per Unit Consisting Of ...` (which itself contains the words `Dividend Re 0.32 Per Unit`, so distributions must be matched before dividends); bond `Interest Payment`; and exotic rights wording (`Rights - 7 Ccps And 7 Warrants:40`). Result: 3,091 parsed / 87 ambiguous / **2 unparsed → 0** after the last two fixes; 62 splits + 66 bonuses + 1 consolidation become 129 factor rows. **Verified against a real bonus:** HDFCBANK 1:1 on 2025-08-26 — raw close 1,964.1 → 973.4 (−50%), adjusted 982.05 → 973.4 (−0.9%).
- **Parser v3 and a composition bug, found 2026-09-20 by scanning the adjusted series for impossible one-day moves** (all 13,705 rows since 2021-01-01; `stk doctor` now runs this scan as `adjusted_jump`):
  - **A split and a bonus on the same ex-date were only half-applied** — 32 events. BAJAJFINSV 2022-09-13 (split 5→1 *and* bonus 1:1) got ×0.2 instead of ×0.1: a phantom 48% crash. Same for BAJFINANCE, 360ONE, NAZARA, LINC. Cause: one factor row per action, two rows sharing an `effective_date`, and the lookup picked one. Same-day actions are now merged into one row (`stk.ingest.adjustments.build_factor_rows`), and a duplicate date raises.
  - **193 subjects v2 left unparsed** (the corpactions job had been ending `failed`). 145 are EGM spellings (`Extra Ordinary` / `Extra-Ordinary` / `Extra- Ordinary` / `Entra Ordinary` / `Extra Oridinary` / `Extra General Meeting` / `... Meting`) — informational. One was a **real bonus, `Bonus- 1:2`** (AJANTPHARM 2022-06-22: adjusted close 1,840.8 → 1,214.8, now 1,227.2 → 1,214.8). The rest: `Divdend`, `Div - Rs`, `Rs - 2.10`, a dividend with no rupee marker, glued words (`Interimdividend`), InvIT `Interest Amount - Rs 1.20 Per Unit/ Return On Capital - ...` payouts (recognised cash, *ambiguous* — no single total, and summing free-text components is a guess), `Capital Reduction` (price-affecting, never carries a ratio → ambiguous, counted), bond `Redemption`, `Bonus Ncrps 1:116` (preference shares → ambiguous). v3 over the stored rows: 160 → parsed, 33 → ambiguous, **0 unparsed**. Rows are `ON CONFLICT DO NOTHING`, so a parser fix never reaches stored rows by re-fetching: `stk ingest corpactions --reparse` re-parses them from `subject_raw`, no network.
- **Rights issues ARE adjusted, since parser v4 (2026-09-21).** The earlier note here said a rights factor "needs the issue price (face value + the `@ Premium` amount) and the cum-price" and left it at that — but the subject carries the ratio *and* the premium (`Rights 3:19 @ Premium Rs 74/-`), the face value is in `securities`, and the cum-rights close is in our own lake, so all three are available. A rights row is now stored **`parsed` with an `issue_premium` and a NULL `price_factor`** (migration `0012`), and `ingest.adjustments` derives the theoretical ex-rights factor: `TERP = (held·P_cum + new·P_issue) / (held + new)`, factor `= TERP / P_cum`. Three things this gets right on purpose, each tested: a rights issue priced at zero reduces to exactly the bonus formula (the strongest check available without market data); rights priced **at or above** the market get factor 1, because nobody subscribes and the price does not drop — derived, not defaulted; and the face value is read **as of the ex-date**, walking back any later face-value change (15 stored rights issues are followed by a split, and today's value would misprice every one). A factor below 0.2 is refused as a misparse rather than applied. **Live result:** 194 of 218 stored rights actions now carry a factor; excluded actions fell **293 → 99**. Verified against real series — GENESYS 2026-08-06 (3:5 @ Rs 45 premium) showed a phantom **−30.3%** one-day crash in the adjusted series and now shows **+2.1%**; JAYKAY 2026-08-28 −9.1% → −1.7%; RATNAVEER 2026-08-25 (a 3% discount) −2.9% → −2.5%. Spellings that all appear and all had to be handled: `Rights Issue 4:17@`, singular `Premium Re 0.45 /-`, `Premium Re. 0.63/-`, and `@ Premium 91` with no currency word.
- **A rights subject that never states a premium stays `ambiguous` (7 rows: `Rights 27:47`, `Rights 1:1`).** Silence is not "at par": a premium NSE simply omitted and a genuine par issue read identically, and assuming par would invent a discount and mark down history that never fell.
- **What is still NOT adjusted (and flagged `degraded` on every rebuild):** **demergers** (71) — a demerger factor needs the value of the entity that was spun off, which no NSE feed gives us — plus 7 premium-less rights, 3 capital reductions (`Capital Reduction Pursuant To Nclt Order`: the subject never carries a ratio), and 1 `Bonus Ncrps 1:116` (a bonus of non-convertible redeemable **preference** shares — it does not dilute the equity count, so no equity factor applies; it is counted only because it is typed `BONUS`). Cash events (dividends, distributions, bond interest) and events with no price effect (buybacks, AGMs) are deliberately *not* counted as degradation; flagging them would raise a permanent false alarm.
- **Ingest behaviour:** every row is stored *first* (unparsed ones as `parse_status='unparsed'`), and the run only then fails, naming every unparsed subject. It used to raise on the first one, which discarded every real split behind it.
- **NSE serves the previous day's file on holidays** (HTTP 200, content dated the day before). `assert_bars_match_requested_date` correctly refuses these; with the holiday calendar loaded (`stk ingest calendar`) those dates are skipped without a request. Verified: all 14 dates that failed a 302-day backfill were genuine NSE holidays per its own holiday master.
- **`sec_bhavdata_full` carries auxiliary series rows beside `EQ`** (`T0`, `P1`, `N1`, `N3`, `E1`, `GS`, ...). WIPRO on 2025-07-08 has an `EQ` row and a `T0` row (3 shares; open=high=low=269.00 but `CLOSE_PRICE` 269.65). Such rows can be internally inconsistent by NSE's own construction, and a symbol-day can have several rows, so anything that needs one bar per stock-day (`backtest_panel`, `panel_for_symbols`) selects one series by an explicit precedence (`EQ > BE > BZ`, `tradeable_series` in `config/backtest.yaml`).
...
### Fundamentals (official filing metadata + XBRL documents)
...
- **Correction to an earlier research assumption:** the build plan's original note ("official, no XBRL parsing needed") assumed `resultDetailedDataLink` would carry pre-parsed structured line items. Live probing found this field **empty on every sampled row** (a broad equities scan and a single-symbol query both). The only structured data actually available without XBRL parsing is **filing metadata** — broadcast date, audited/consolidated flags, period, and a link to the underlying XBRL document (`xbrl` field) — not income/balance/cashflow figures themselves. `NseFundamentalsProvider` is implemented against this reality: `fetch_filings_index`/`fetch_statements` return this metadata (`statement_type="meta"` in `fundamentals_snapshots`, honestly labelled rather than pretending it's parsed financials), with the XBRL link preserved in `data`/`source_url` for a future parsing pass. Line-item XBRL parsing was subsequently implemented in Phase 3 — see the next bullet.
- **XBRL documents — parsed since Phase 3 (`stk.ingest.xbrl`), verified live 2026-09-19** against Reliance Q3 FY25 and FY24 annual, both consolidated (real fixtures in `tests/fixtures/nse/xbrl/`). `https://nsearchives.nseindia.com/corporate/xbrl/INDAS_*.xml` needs only a browser User-Agent (no cookie). The listing API also worked without a cookie. Findings that shaped the parser — none of them obvious, all of them silent-corruption risks if missed:
  - **Context IDs carry the meaning; the printed dates cannot be trusted.** The template defines three plain (no-scenario) contexts: `OneD` = the discrete quarter, `FourD` = the year-to-date (the *full year* in an annual filing), `OneI` = the period-end balance sheet. `FourD` prints **exactly the same start/end dates as `OneD`** yet holds the cumulative values (Reliance Q3 FY25: revenue 1.28 trn under `OneD`, 3.97 trn under `FourD`). Reading meaning from the dates would store nine months as a quarter.
  - **Every filing's `OneD` is already a discrete quarter**, and an annual filing's `OneD` is Q4 — so trailing-twelve-month sums need no "FY minus nine months" derivation.
  - **Quarterly filings carry no balance sheet** (no equity, no borrowings); half-year and annual ones do. ROCE and D/E therefore use the latest filing that had one.
  - **`DebtEquityRatio` is unusable**: it reads `0.00` for Reliance while the same annual filing shows Rs 3.2 trn of borrowings. D/E is derived from `BorrowingsNoncurrent + BorrowingsCurrent` over `Equity` instead.
  - Element names (`RevenueFromOperations`, `ProfitBeforeTax`, `ProfitLossForPeriod`, `FinanceCosts`, `Equity`, `BorrowingsNoncurrent/Current`, EPS) are stable across both filings and are matched by *local name*; the map is `config/xbrl_tags.yaml`.
  - **Banks (`bank: "Y"`) and pre-Ind-AS filings use a different taxonomy** and are recorded `unsupported_format` with no line items — not guessed at. **Only 2 of the many template variants have been observed**; expect the `partial`/`unsupported_format` counts from a real sweep to reveal more, and read them before trusting a strategy that leans on fundamentals.
  - **History depth is shallow**: the annual listing for RELIANCE began at FY2021. 3-year CAGRs therefore exist only from ~FY2024, so the long-term strategy has very few usable windows and is always labelled approximate. Whether `from_date`/`to_date` parameters reach further back has **not** been tested.
- **The legacy feed stopped at the Dec 2024 quarter (found live 2026-09-19).** `corporates-financial-results` has no period_end after 2024-12-31 (its newest broadcasts are April 2025, plus two 2026 re-filings of old periods), so as of Sep 2026 every fundamental built from it was ~20 months stale and the long-term strategy's `fund_age_days <= 200` could never hold. NSE moved to the **Integrated Filing** system.
- **Integrated Filing endpoint (verified live 2026-09-19; browser UA only, no cookie):** `https://www.nseindia.com/api/integrated-filing-results?index=equities&period_ended=all&type=Integrated%20Filing-%20Financials&symbol=RELIANCE&size=200` → `{"data": [...], "size", "page", "totalCount"}` (26,747 rows market-wide). Each row: `symbol`, `qe_Date` (period end, upper-case month), `consolidated`, `audited`, `type_Sub` (`Original` | `Revision`), `broadcast_Date` (**null on revisions** → fall back to `revised_Date`, then `creation_Date`), `seq_Id`, and `xbrl` / `ixbrl` links; **no ISIN and no Quarterly/Annual flag**. RELIANCE returns 12 rows (six quarters Mar 2025–Jun 2026 × standalone/consolidated). Without `symbol=` the default page is 20 rows and `size=` widens it. The documents (`.../corporate/xbrl/INTEGRATED_FILING_INDAS_*_WEB.xml`) use **the same element names and the same `OneD`/`FourD`/`OneI` contexts** as the legacy ones, so the existing parser handles them unchanged: Reliance's March-2026 filing gives the quarter (`OneD`, 2,986 bn revenue), the FULL YEAR (`FourD`, 10,757 bn) and a balance sheet; the June filing gives the quarter only. Hence a March filing is stored as `FY` and the rest as `Q`. Revisions arrive as separate rows and can be months or years late (INTERARCH's FY25 revision was filed 2026-09-19); each is kept, and from its own broadcast date onward it supersedes the original. Implemented in `NseFundamentalsProvider.fetch_integrated_statements`; `stk ingest fundamentals-sweep [--integrated-only]`.
